...
def conv_layer(img, n_filters, filter_size):
    n_channels = np.shape(img)[-1]
    filter = tf.get_variable('weights',shape = [filter_size, filter_size, n_channels, n_filters], initializer = tf.random_normal_initializer(0,0.1) )
    return tf.nn.conv2d(img, filter,[1,1,1,1], 'SAME')